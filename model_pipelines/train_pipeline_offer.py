...
df, df_valid = train_test_split(df0, test_size=0.1, random_state=42, stratify=df0['ofr_id_short'])
X = df.drop(columns=['person', 'ofr_id_short'])
y = df['ofr_id_short']
...
model_path = f'../saved_models/model_ofr_id_short_{best_model.named_steps["classifier"].__class__.__name__}.pkl'
...
print_metrics2(loaded_model, y=df_valid['ofr_id_short'], y_pred=valid_predicted)

valid_table = df_valid[['gender']].copy()
valid_table['ofr_id_short_original'] = df_valid['ofr_id_short']
valid_table['ofr_id_short_predicted'] = valid_predicted
display(valid_table)
