...
df0 = df0.drop(columns=['person', 'became_member_on', 'bec_memb_year_month', 'tag'])

df_unknown = df0[df0['gender'].isna()] #unknown gender to be predicted

df = df0[~df0['gender'].isna()]
...
y = pd.get_dummies(df['ofr_id_short'], dtype=int)
